...
현재 yt-dlp의 YouTube EJS 경로는 외부 JavaScript 런타임을 필요로 한다. yttStudio v0.3.0은 아래 Deno 정책으로 해당 요구를 충족하고, 직접 사전 확인에는 `--js-runtimes deno:<path>`를 명시한다. 이 옵션을 지원하지 않는 기존 yt-dlp는 재사용하지 않고 위 고정 자산으로 대체한다.
