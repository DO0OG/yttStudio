...
현재 yttStudio 릴리스 패키지에는 `yt-dlp`, `libmpv` 또는 KMediaMpv 런타임 바이너리를 직접 포함하지 않는다.
...
`libmpv = LGPL`이라고 일반화하지 않고 **자동 설치 대상으로 고정한 정확한 산출물**을 기준으로 판단한다. v0.2.3은 과거 Shinchiro 기본 빌드를 자동 설치 대상으로 사용하지 않는다.
...
릴리스 워크플로는 최종 publish 디렉터리에서 yt-dlp standalone과 `libmpv*`, `libkmediampv_*`, KMediaMpv runtime JAR이 발견되면 실패하도록 구성한다.
