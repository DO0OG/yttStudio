...
| | |
|---|---|
| **v0.3.0 複数キーフレーム移動** | 位置の移動が始点と終点の 2 点補間から、複数のキーフレームをつなぐ経路に広がりました。プレビューに経路とキーフレームのマーカーが表示され、マーカーをドラッグして位置を変更でき、タイムラインでキーフレームの時刻を確認できます |
| **リリースの自動更新** | 新しいバージョンが公開されると通知し、更新を選ぶと現在の OS に合うファイルをダウンロードしてインストールまで行います。起動時に確認するかどうかは設定で切り替えられます |
| **複数行字幕の編集** | キュー一覧が字幕の行数に合わせて行の高さを広げ、複数行のテキストをそのまま表示して編集できます。一度に表示する最大行数は設定で決めます |
