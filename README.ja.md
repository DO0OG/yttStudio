...
**ビューポートモード (通常・シアター・全画面・モバイル) を選べます。** YouTube 上で各モードの
プレイヤーと字幕のサイズを実測し、その値で実装しました。字幕の文字サイズはプレイヤーの
**幅**に比例し、通常とシアターは相似形です。計測方法と生の数値は
[ビューポートモード実測記録](docs/viewport-modes.md) にあります。全画面とモバイル縦は未計測で、
通常モードの比率をそのまま使います。
