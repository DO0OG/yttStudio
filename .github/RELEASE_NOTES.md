...
| yt-dlp | 유튜브 주소로 열기 | 파일로 연 영상은 그대로 동작합니다. 개발 빌드는 앱 폴더 또는 `PATH`에 준비하세요 |

libmpv는 번들하지 않으며 앱의 **도구 → 설정 → 영상** 에서 내려받을 수 있습니다. 릴리즈
패키지에는 공식 yt-dlp `2026.08.19` 바이너리가 `SHA2-256SUMS`로 검증되어 앱 폴더에
번들됩니다. 개발 빌드는 앱 폴더나 `PATH`에서 `yt-dlp`를 찾고, 자동 설치·업데이트는
없습니다. 자세한 사정은 `docs/DEPENDENCIES.md`에 있습니다.
