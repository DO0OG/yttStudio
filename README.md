...
**뷰포트 모드(일반·극장·전체화면·모바일)를 고를 수 있습니다.** 유튜브에서 각 모드의 플레이어와
자막 크기를 실측한 뒤 그 값으로 구현했습니다. 자막 글자 크기는 플레이어 **너비**에 비례하며,
일반과 극장 모드는 서로 닮은꼴입니다. 측정 방법과 원본 수치는
[뷰포트 모드 실측 기록](docs/viewport-modes.md)에 있습니다. 전체화면과 모바일 세로는 아직
실측하지 못해 일반 모드의 비례식을 그대로 씁니다.
