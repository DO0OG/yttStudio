...
| | |
|---|---|
| **v0.3.0 다중 키프레임 이동** | 위치 이동이 시작점과 끝점 두 점을 잇는 보간에서 여러 키프레임을 잇는 경로로 넓어졌습니다. 프리뷰 화면에 경로와 키프레임 마커가 보이고 마커를 끌어 위치를 옮기며 타임라인에서 키프레임 시각을 확인합니다 |
| **릴리스 자동 업데이트** | 새 버전이 올라오면 알리고, 업데이트를 고르면 지금 쓰는 운영체제에 맞는 파일을 내려받아 설치까지 진행합니다. 시작할 때 확인할지 여부는 설정에서 끌 수 있습니다 |
| **여러 줄 자막 편집** | 큐 목록이 자막 줄 수만큼 행 높이를 늘려 여러 줄 자막을 그대로 보여주고 편집합니다. 한 번에 보여줄 최대 줄 수는 설정에서 정합니다 |
| **v0.2.5 YouTube 재생 오류 수정** | YouTube 주소 재생 준비 중 Deno 설치 파일이 잠겨 항상 실패하던 문제를 고쳤습니다. `--js-runtimes`를 지원하지 않는 오래된 yt-dlp는 공식 고정 자산으로 자동 대체합니다 |
...
| yt-dlp | YouTube 주소 해석에 필요합니다. `--js-runtimes`를 지원하는 기존 설치본을 우선 사용하고, 없거나 지원하지 않으면 공식 `yt-dlp/yt-dlp 2026.08.19` 자산을 SHA-256 검증 후 사용자 영역에 자동 설치합니다 |
